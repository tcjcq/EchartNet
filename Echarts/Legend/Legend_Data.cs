...
public class Legend_Data
{
	...
	{
		// 获取所有需要检查的属性（排除Name属性）
		var properties = GetType().GetProperties()
			.Where(p => p.Name != nameof(Name));
		...
		foreach (var prop in properties)
		{
			...
			{
				var son = (StringOrNumber)value;
				if (son != null && !IsDefaultStringOrNumber(son))
					return false;
				continue;
			}

			// 处理默认值逻辑
			if (!IsDefaultValue(value, prop))
				return false;
		}
	...
	private bool IsDefaultValue(object value, PropertyInfo prop)
	{
		// 处理字符串类型
		if (prop.PropertyType == typeof(string))
			return string.IsNullOrEmpty((string)value);

		// 处理特殊默认值（如InactiveBorderWidth的"auto"）
		if (prop.Name == nameof(InactiveBorderWidth))
			return value == null || value.ToString() == "auto";

		// 引用类型默认判断
		return value == null || value.Equals(GetDefault(prop.PropertyType));
	}

	private bool IsDefaultStringOrNumber(StringOrNumber son)
	{
		return son is not { Value: not null } ||
		       (son.Value is string s && string.IsNullOrEmpty(s));
	}
