...
namespace Echarts
{
	...
	public class Tooltip
	{
		...
		/// <summary>
		///     坐标轴指示器配置项。
		///     tooltip.axisPointer 是配置坐标轴指示器的快捷方式。实际上坐标轴指示器的全部功能，都可以通过轴上的 axisPointer 配置项完成（例如 xAxis.axisPointer 或
		///     angleAxis.axisPointer）。但是使用 tooltip.axisPointer 在简单场景下会更方便一些。
		...
		///     设置 tooltip.trigger 设置为 'axis' 或者 tooltip.axisPointer.type 设置为 'cross'，则此时坐标系会自动选择显示哪个轴的 axisPointer，也可以使用
		///     tooltip.axisPointer.axis 改变这种选择。注意，轴上如果设置了 axisPointer，会覆盖此设置。
		...
		///     注意：
		///     如果发现此时 tooltip 效果不良好，可设置 tooltip.triggerOn 为 'none'（于是效果为：手指按住按钮则显示 tooltip，松开按钮则隐藏 tooltip），或者
		...
		/// </summary>
		[JsonProperty("alwaysShowContent")]
		...
		///     同时鼠标移动和点击时触发。
		///     'none'
		///     不在 'mousemove' 或 'click' 时触发，用户可以通过 action.tooltip.showTip 和 action.tooltip.hideTip 来手动触发和隐藏。也可以通过
		///     axisPointer.handle 来触发或隐藏。
		///     该属性为 ECharts 3.0 中新加。
		/// </summary>
		...
		/// </summary>
		[JsonProperty("hideDelay")]
		public double? HideDelay { get; set; }

		/// <summary>
		///     鼠标是否可进入提示框浮层中，默认为false，如需详情内交互，如添加链接，按钮，可设置为 true。
		/// </summary>
		[JsonProperty("enterable")]
		public bool? Enterable { get; set; }

		/// <summary>
		///     浮层的渲染模式，默认以 'html 即额外的 DOM 节点展示 tooltip；此外还可以设置为 'richText' 表示以富文本的形式渲染，渲染的结果在图表对应的 Canvas 中，这对于一些没有 DOM
		///     的环境（如微信小程序）有更好的支持。
		/// </summary>
		...
		/// </summary>
		[JsonProperty("confine")]
		public bool? Confine { get; set; }

		/// <summary>
		///     从 v4.7.0 开始支持
		///     （自 v5.5.0 已废弃，请使用 appendTo。）
		///     是否将 tooltip 的 DOM 节点添加为 HTML 的
		///     <body>
		///         的子节点。只有当 renderMode 为 'html' 是有意义的。
		///         默认值是 false。false 表示，tooltip 的 DOM 节点会被添加为本图表的 DOM container 的一个子孙节点。但是这种方式导致，如果本图表的 DOM container 的祖先节点有设置
		///         overflow: hidden，那么当 tooltip 超出 container 范围使可能被截断。这个问题一定程度上可以用 tooltip.confine 来解决，但是不一定能解决所有场景。
		///         所以这里我们提供了 appendToBody: true 来解决这件事。这也是常见的解决此类问题的一种方式。但是 true 并不定为默认值，因为要避免 break change，尤其是一些对于 tooltip
		///         深入定制的使用。并且也避免一些未知的 bad case。
		///         注：CSS transform 的场景，这也可以使用。
		/// </summary>
		[JsonProperty("appendToBody")]
		public bool? AppendToBody { get; set; }

		/// <summary>
		///     从 v5.5.0 开始支持
		///     将 tooltip 的 DOM 节点添加到哪个节点下。只有当 renderMode 为 'html' 是有意义的。
		///     默认值是 null，表示 tooltip 的 DOM 节点会被添加为本图表的 DOM container 的一个子孙节点。但是这种方式导致，如果本图表的 DOM container 的祖先节点有设置 overflow:
		///     hidden，那么当 tooltip 超出 container 范围使可能被截断。这个问题一定程度上可以用 tooltip.confine 来解决，但是不一定能解决所有场景。
