...
public class Dataset
{
	...
	///     参见 数据变换教程
	/// </summary>
	[JsonProperty("fromDatasetIndex")]
	public double? FromDatasetIndex { get; set; }

	/// <summary>
	///     指定 dataset.transform 以哪个 dataset 作为输入。
	///     参见 数据变换教程
	/// </summary>
	[JsonProperty("fromDatasetId")]
	public string FromDatasetId { get; set; }

	/// <summary>
	///     如果一个 dataset.transform 会产出多个结果 data ，我们可以使用 fromTransformResult 获得特定的结果。
	///     大多数场景下，transform 只会产出一个结果，所以大多数情况下 fromTransformResult 并不需要指定。当不指定 fromTransformResult 时，默认使用 fromTransformResult:
	///     0。
	///     参见 数据变换教程
	/// </summary>
	[JsonProperty("fromTransformResult")]
