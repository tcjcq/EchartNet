...
namespace Echarts
{
	...
	public class SeriesTree
	{
		...
		/// <summary>
		///     树图的布局，有 正交 和 径向 两种。这里的 正交布局 就是我们通常所说的 水平 和 垂直 方向，对应的参数取值为 'orthogonal' 。而 径向布局
		...
		/// <summary>
		///     树图中 正交布局 的方向，也就是说只有在 layout = 'orthogonal' 的时候，该配置项才生效。对应有 水平 方向的 从左到右，从右到左；以及垂直方向的 从上到下，从下到上。取值分别为 'LR' , 'RL',
		///     'TB', 'BT'。注意，之前的配置项值 'horizontal' 等同于 'LR'， 'vertical' 等同于 'TB'。
		/// </summary>
		[JsonProperty("orient")]
		...
		///     常规的标记类型，如 'emptyCircle' 等。
		/// </summary>
		[JsonProperty("expandAndCollapse")]
		public bool? ExpandAndCollapse { get; set; }

		/// <summary>
		///     树图初始展开的层级（深度）。根节点是第 0 层，然后是第 1 层、第 2 层，... ，直到叶子节点。该配置项主要和 折叠展开 交互一起使用，目的还是为了防止一次展示过多节点，节点之间发生遮盖。如果设置为 -1 或者 null
		///     或者 undefined，所有节点都将展开。
		/// </summary>
