...
{
	/// <summary>
	///     数据集合（支持一维值、二维坐标、对象配置）
	/// </summary>
	[JsonProperty("data")]
	[JsonConverter(typeof(DataItemsConverter))]
...
{
	public List<StringOrNumber> Data { get; set; } = [];
	...
	/// </summary>
	[JsonProperty("value")]
	public StringOrNumber Value { get; set; }
	...
	/// <summary>
	///     从简单值隐式转换（如 23 → DataItem）
	/// </summary>
	public static implicit operator DataItem(int value)
	{
		return new DataItem { Value = value };
	}

	public static implicit operator DataItem(double value)
	{
		return new DataItem { Value = value };
	}
	...
	public static implicit operator DataItem(List<StringOrNumber> value)
	{
		return new DataItem { Data = value };
	}
}
...
///     处理混合类型数据项的JSON转换器
/// </summary>
public class DataItemsConverter : JsonConverter
{
	public override bool CanConvert(Type objectType)
	{
		return objectType == typeof(List<DataItem>);
	}
	...
	public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
	{
		...
		foreach (var token in array)
			if (token.Type == JTokenType.Object)
				// 对象格式：{ value: ..., label: ... }
				items.Add(token.ToObject<DataItem>());
			else
				// 简单值或数组
				items.Add(new DataItem { Value = token.ToObject<StringOrNumber>() });
		return items;
	}

	public override void WriteJson(JsonWriter writer, object value, JsonSerializer serializer)
	{
		var list = (List<DataItem>)value;
		var array = new JArray();

		foreach (var item in list)
			if (item.Label != null || item.ItemStyle != null)
			{
				// 序列化为对象
				var obj = new JObject
				{
					["value"] = item.Value != null ? JToken.FromObject(item.Value) : JValue.CreateNull()
				};
				if (item.Label != null) obj["label"] = JToken.FromObject(item.Label);
				if (item.ItemStyle != null) obj["itemStyle"] = JToken.FromObject(item.ItemStyle);
				array.Add(obj);
			}
			else
			{
				// 直接序列化值
				array.Add(item.Value != null ? JToken.FromObject(item.Value) : JValue.CreateNull());
			}
