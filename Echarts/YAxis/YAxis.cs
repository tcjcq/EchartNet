...
public class YAxis
{
	...
	public List<StringOrNumber> BoundaryGap { get; set; }
	...
	///     return value.min - 20;
	///     }
	///     其中 value 是一个包含 min 和 max 的对象，分别表示数据的最大最小值，这个函数可返回坐标轴的最小值，也可返回 null/undefined 来表示“自动计算最小值”（返回 null/undefined 从
	///     v4.8.0 开始支持）。
	/// </summary>
	[JsonProperty("min")]
	...
	///     return value.max - 20;
	///     }
	///     其中 value 是一个包含 min 和 max 的对象，分别表示数据的最大最小值，这个函数可返回坐标轴的最大值，也可返回 null/undefined 来表示“自动计算最大值”（返回 null/undefined 从
	///     v4.8.0 开始支持）。
	/// </summary>
	[JsonProperty("max")]
	...
	public YAxis_AxisLabel AxisLabel { get; set; }
