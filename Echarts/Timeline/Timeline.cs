...
namespace Echarts
{
	...
	///     初始化的时候，对应于当前时间的那个 switchableOption 会被合并（merge）到 baseOption，形成 finalOption。而每当时间变化时，对应于新时间的 switchableOption
	///     会被合并（merge）到finalOption。
	///     有两种合并（merge）策略：
	///     默认使用 NORMAL_MERGE。
	...
	public class Timeline
	{
		...
		///     常见需要使用 replaceMerge 的地方是，如果需要下一个时间刻度的 series 完全替换上一个时间刻度的 series 而不进行任何 merge ，可以设置 replaceMerge:
		///     'series'，并且两个时间刻度的 series id 不相同或者没有 id 。
		///     参见这个 示例。
		/// </summary>
		...
		/// <summary>
		///     timeline标记相对于原本位置的偏移。默认情况下，标记会居中置放在数据对应的位置，但是如果 symbol 是自定义的矢量路径或者图片，就有可能不希望 symbol 居中。这时候可以使用该配置项配置 symbol
